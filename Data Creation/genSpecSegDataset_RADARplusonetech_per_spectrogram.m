...
%% -------- Dataset knobs --------
Nmc = 100;                       % number of spectrogram/mask pairs to generate
...
% -------- One-tech-per-image schedule (exact proportions when possible) --------
% -------- Non-RADAR tech schedule: WLAN/NR/LTE each with 1/3 probability --------
otherClasses = ["NR","LTE","WLAN"];
wOther = [1/3 1/3 1/3];

expCounts  = wOther * Nmc;
baseCounts = floor(expCounts);
remN       = Nmc - sum(baseCounts);

[~,ord] = sort(expCounts - baseCounts, 'descend');
baseCounts(ord(1:remN)) = baseCounts(ord(1:remN)) + 1;

otherPerTrial = strings(Nmc,1);
idx = 1;
for c = 1:numel(otherClasses)
    if baseCounts(c) > 0
        otherPerTrial(idx:idx+baseCounts(c)-1) = otherClasses(c);
        idx = idx + baseCounts(c);
    end
end
otherPerTrial = otherPerTrial(randperm(Nmc));   % shuffle
...
for i = 1:Nmc
    ...
    % ==========================================================
    % ==========================================================
    % 1) Always include 3 RADAR stripes + 1 non-RADAR technology
    % ==========================================================
    otherChosen = string(otherPerTrial(i));   % "NR"/"LTE"/"WLAN"
    
    Krad = numRadarRegions;                  % 3
    labelsTrue  = [repmat("RADAR", Krad, 1); otherChosen];  % (Krad+1) bands
    bandsTrueHz = zeros(Krad+1, 2);
    f0ListHz    = zeros(Krad+1, 1);
    
    % ==========================================================
    % 2) Place RADAR stripes first (non-overlapping with guard)
    % ==========================================================
    BWkR  = BW.RADAR;
    f0minR = fMin + BWkR/2 + guardHz;
    f0maxR = fMax - BWkR/2 - guardHz;
    if f0minR >= f0maxR
        error('No room to place RADAR bands. Reduce guardHz/BW or expand [fMin,fMax].');
    end
    
    for r = 1:Krad
        placed = false;
        for attempt = 1:600
            f0 = f0minR + (f0maxR - f0minR)*rand();
            fL = f0 - BWkR/2;
            fH = f0 + BWkR/2;
        ...
        if ~placed
            error('Could not place %d RADAR stripes. Reduce guardHz or BW.RADAR.', Krad);
        end
    end
    
    % ==========================================================
    % 3) Place ONE non-RADAR band (must not overlap any RADAR stripe)
    % ==========================================================
    BWkO   = BW.(otherChosen);
    f0minO = fMin + BWkO/2 + guardHz;
    f0maxO = fMax - BWkO/2 - guardHz;
    if f0minO >= f0maxO
        error('No room to place %s band. Reduce guardHz/BW or expand [fMin,fMax].', otherChosen);
    end
    
    placed = false;
    for attempt = 1:600
        f0 = f0minO + (f0maxO - f0minO)*rand();
        fL = f0 - BWkO/2;
        fH = f0 + BWkO/2;
    
        prev = bandsTrueHz(1:Krad, :); % radar bands
        ok = all( (fH < prev(:,1)-guardHz) | (fL > prev(:,2)+guardHz) );
    
        if ok
            bandsTrueHz(Krad+1,:) = [fL fH];
            f0ListHz(Krad+1)      = f0;
            placed = true;
            break;
        end
    ...
    if ~placed
        error('Could not place %s band without overlapping RADAR. Reduce guardHz or BW.', otherChosen);
    end
