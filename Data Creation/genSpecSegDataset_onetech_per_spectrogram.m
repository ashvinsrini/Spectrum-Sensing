...
classes  = ["NR","LTE","WLAN","RADAR"];
weights  = [0.25  0.25   0.25   0.25];      % desired probabilities
weights  = weights / sum(weights);

expCounts  = weights * Nmc;
baseCounts = floor(expCounts);
remN       = Nmc - sum(baseCounts);

% distribute leftover samples by largest fractional parts
[~,ord] = sort(expCounts - baseCounts, 'descend');
baseCounts(ord(1:remN)) = baseCounts(ord(1:remN)) + 1;

techPerTrial = strings(Nmc,1);
idx = 1;
for c = 1:numel(classes)
    if baseCounts(c) > 0
        techPerTrial(idx:idx+baseCounts(c)-1) = classes(c);
        idx = idx + baseCounts(c);
    end
end

% shuffle order so classes are mixed
techPerTrial = techPerTrial(randperm(Nmc));
...
for i = 1:Nmc
    ...
    % ==========================================================
    % ==========================================================
    % 1) Pick exactly ONE technology for this spectrogram
    % ==========================================================
    techChosen = string(techPerTrial(i));   % "NR"/"LTE"/"WLAN"/"RADAR"
    
    % ==========================================================
    % 2) Allocate band(s) for that technology only
    %    - if RADAR: allow numRadarRegions bands (currently 1)
    %    - else: single band
    % ==========================================================
    if techChosen == "RADAR"
        K = numRadarRegions;  % set to 1 for now (can increase later)
        ...
        for r = 1:K
            ...
            for attempt = 1:300
                ...
                if r == 1
                    ...
                    % ensure non-overlap with previous RADAR regions (plus guard)
            ...
            if ~placed
                error('Could not place RADAR band(s). Reduce guardHz/BW or expand [fMin,fMax].');
            end
        end
    
    else
        labelsTrue  = techChosen;    % single band
        BWk   = BW.(techChosen);
        f0min = fMin + BWk/2 + guardHz;
        f0max = fMax - BWk/2 - guardHz;
    
        if f0min >= f0max
            error('No room to place %s band. Reduce guardHz/BW or expand [fMin,fMax].', techChosen);
        end
